...
public interface EbookRepository extends JpaRepository<EBook, UUID> {
    ...
    @Query
            ("SELECT DISTINCT e FROM EBook e JOIN e.authors a WHERE LOWER(a.firstName) LIKE LOWER(CONCAT('%',:q,'%'))" +
                    "OR LOWER(a.lastName) LIKE LOWER(CONCAT('%',:q,'%')) "
            )
    List<EBook> findByAuthorNameOrSurnameLike(@Param("q") String q);

    @Query("SELECT DISTINCT e FROM EBook e  " +
            "JOIN e.authors a " +
            "WHERE LOWER(a.firstName) = LOWER(:firstName)" +
            "AND LOWER(a.lastName)= LOWER(:lastName)")
    List<EBook> findByAuthorFullName(@Param("firstName") String firstName, @Param("lastName") String lastName);
